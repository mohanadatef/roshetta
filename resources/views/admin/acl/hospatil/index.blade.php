...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url('/admin/hospatil/index') }}"><i class="fa fa-Users"></i>{{ trans('lang.Index') }}</a></li>
        </ol>
    </section>
    <section class="content">
        <form id="status" method="get" action="{{ url('/admin/hospatil/change_many_status')}}">
            <div class="box">
                <div class="box-header" align="right">
                    @if(count($datas) > 0)
                        @if(permission_show('hospatil-many-status'))
                        ...
                        <div align="center" class="col-md-12 table-responsive">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    @if(permission_show('hospatil-status'))
                                ...
                                @foreach($datas as $data)
                                    <tr>
                                        ...
                                        @if(permission_show('hospatil-status'))
                                            <td align="center">
                                                @if($data->status ==1)
                                                    <a href="{{ url('/admin/hospatil/change_status/'.$data->id)}}"><i
                                        ...
                                        @if(permission_show('hospatil-status-request'))
                                            <td align="center">
                                                @if($data->hospatil->status_request ==1)
                                                    <a href="{{ url('/admin/hospatil/change_status_request/'.$data->hospatil->id)}}"><i
                                                                class="btn btn-danger ace-icon fa fa-close">{{ trans('lang.An_active') }}</i></a>
                                                @elseif($data->hospatil->status_request ==0)
                                                    <a href="{{ url('/admin/hospatil/change_status_request/'.$data->hospatil->id)}}"><i
                                                                class="btn btn-primary ace-icon fa fa-check-country"> {{ trans('lang.Active') }}</i></a>
                                                @endif
                                            </td>
...
@section('script_style')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Acl\Hospatil\StatusEditRequest','#status') !!}
@endsection