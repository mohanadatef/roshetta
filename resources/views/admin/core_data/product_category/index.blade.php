...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url('/admin/product_category/index') }}"><i class="fa fa-product_categorys"></i>{{ trans('lang.Index') }}</a></li>
        </ol>
    </section>
    <section class="content">
        <form method="get" id="status" action="{{ url('/admin/product_category/change_many_status')}}">
            <div class="box">
                <div class="box-header" align="right">
                    @if(permission_show('product-category-create'))
                    <a href="{{  url('/admin/product_category/create') }}" class="btn btn-primary">{{ trans('lang.Create') }}</a>
                    @endif
                        @if(count($datas) > 0)
                        ...
                        <div align="center" class="col-md-12 table-responsive">
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    @endif
                                    @if(permission_show('product-category-edit'))
                                ...
                                @foreach($datas as $data)
                                    <tr>
                                        ...
                                        <td align="center">
                                                @if($data->status ==1)
                                                    <a href="{{ url('/admin/product_category/change_status/'.$data->id)}}"><i
                                                                class="btn btn-danger ace-icon fa fa-close">{{ trans('lang.An_active') }}</i></a>
                                                @elseif($data->status ==0)
                                                    <a href="{{ url('/admin/product_category/change_status/'.$data->id)}}"><i
                                                                class="btn btn-primary ace-icon fa fa-check-product_category"> {{ trans('lang.Active') }}</i></a>
                                                @endif
                                        </td>
                                            @endif
                                            @if(permission_show('product-category-edit'))
                                        <td align="center">
                                                <a href="{{ url('/admin/product_category/edit/'.$data->id)}}"><i
...
@section('script_style')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Core_Data\Product_Category\StatusEditRequest','#status') !!}
@endsection