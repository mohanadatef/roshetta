...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url('/admin/medicine/index') }}"><i class="fa fa-permsissions"></i>{{ trans('lang.Index') }}</a></li>
            <li><a href="{{ url('/admin/medicine/create') }}"><i class="fa fa-permsission"></i>{{ trans('lang.Create') }}</a>
            </li>
        </ol>
    </section>
    <section class="content">
        <div class="box">
            ...
            <!-- /.box-header -->
            <div class="box-body">
                <form id='create' action="{{url('admin/medicine/store')}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
                    ...
                    <div class="form-group{{ $errors->has('medicine_category_id') ? ' has-error' : "" }}">
                        ...
                        <select id="medicine_category" class="form-control select2" data-placeholder="{{trans('lang.Medicine_Category')}}" name="medicine_category_id">
                            <option value="0" selected>{{trans('lang.Select')}}</option>
                            @foreach($medicine_category as  $mymedicine_category)
                                <option value="{{$mymedicine_category->id}}"> {{$mymedicine_category->title}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group{{ $errors->has('price') ? ' has-error' : "" }}">
                        {{ trans('lang.Price') }} : <input type="text" value="{{Request::old('price')}}"
...
@section('script_style')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Core_Data\Medicine\CreateRequest','#create') !!}
@endsection