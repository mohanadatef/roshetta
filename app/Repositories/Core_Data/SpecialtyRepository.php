...
class SpecialtyRepository implements SpecialtyInterface
{
    ...
    public function Get_One_Data_Translation($id)
    {
        ...
        return $specialty ? array_merge($this->specialty->find($id)->toarray(), $specialty) : $this->specialty->find($id);
    }
    ...
    public function Update_Status_One_Data($id)
    {
        ...
        $specialty->status == 1 ? $specialty->status = '0' : $specialty->status = '1';
        $specialty->update();
    }
    ...
    {
        foreach ($this->Get_Many_Data($request) as $specialty) {
            $specialty->status == 1 ? $specialty->status = '0' : $specialty->status = '1';
            $specialty->update();
        }
    }
